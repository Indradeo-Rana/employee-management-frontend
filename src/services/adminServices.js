...
export const attendanceService = {
  // Monthly register (new)
  getRegister: (year, month) =>
    axiosInstance.get(`/api/admin/attendance/register?year=${year}&month=${month}`).then(r => r.data.data),

  // Save single cell (new)
  saveCell: (data) =>
    axiosInstance.post("/api/admin/attendance/cell", data).then(r => r.data),

  // Existing
  bulkMark:     (data)           => axiosInstance.post("/api/admin/attendance/bulk", data).then(r => r.data.data),
  markSingle:   (data)           => axiosInstance.post("/api/admin/attendance/single", data).then(r => r.data.data),
